...
public class Game implements GameManager {
	...
	/**
	 * Allows the current player to select the given marble. Ensures that the player
	 * does not select more than the allowed number of marbles.
	 *
	 * @param marble The marble to be selected.
	 * @throws InvalidMarbleException if the player tries to select an invalid
	 *                                marble or exceeds the allowed number of
	 *                                selected marbles.
	 */
	...
	/**
	 * Clears all selections made by the current player. This method resets the
	 * selected card and any selected marbles, ensuring a fresh selection for the
	 * player's turn.
	 */
	...
	/**
	 * Sets the splitDistance to the given value, ensuring it is within the valid
	 * range (1-6). Throws a SplitOutOfRangeException if the value is out of range.
	 * 
	...
	public void editSplitDistance(int splitDistance) throws SplitOutOfRangeException {
		if (splitDistance >= 1 && splitDistance <= 6)
			...
			throw new SplitOutOfRangeException("You're not playing Monopoly — splits can only go from 1 to 6!");
	}

	/**
	 * Determines if the player's turn should be skipped based on their hand card
	 * count and the current turn number
	 * 
	...
	/**
	 * Ends the current player's turn after they have either played a card or their
	 * turn was skipped. The method performs the following tasks: 1. Removes the
	 * selected card from the player's hand and adds it to the firePit. 2. Deselects
	 * any currently selected card by the player. 3. Advances to the next player in
	 * the turn order, wrapping around if necessary. 4. Starts a new round if all
	 * players have completed a turn. 5. Refills all players' hands from the deck
	 * when starting a new round. 6. Refills the deck's card pool from the firePit
	 * if the deck has fewer than 4 cards.
	 */
	public void endPlayerTurn() {
		...
		// This could result in unintended behavior (e.g., adding null cards to the fire
		// pit).
		if (x != null) {
	 ...
	 * A player wins when their safe zone is full, and this method checks each
	 * player's safe zone to determine if that condition is met.
	 * 
	 * @return the colour of the player who has won the game, or null if no player
	 *         has won yet.
	 */
	...
	/**
	 * Fields a marble from the current player's collection to their Base Cell on
	 * the board.
	 * 
	 * This method retrieves one marble from the current player's Home Zone. If the
	 * player has no marbles to field, a {@link CannotFieldException} is thrown. If
	 * the player has a marble, it is sent to the player's Base Cell on the board.
	 * The marble is then removed from the player's Home Zone (collection of
	 * marbles).
	 * 
	 * @throws CannotFieldException    If the current player has no marble to field.
	 * @throws IllegalDestroyException If an opponent's marble is destroyed while
	 *                                 fielding the marble.
	 */
	...
	/**
	 * Discards a random card from the player with the given colour. If the player
	 * with the specified colour does not exist or has no cards in hand, a
	 * CannotDiscardException is thrown.
	 * 
	 * @param colour The colour of the player from whom the card is to be discarded.
	 * @throws CannotDiscardException If the player has no cards to discard or if
	 *                                the player with the given colour is not found.
	 */
	...
	/**
	 * Discards a random card from a player other than the current player. This
	 * method generates a random index (from 0 to 3) and calls the discardCard
	 * method for the player at the generated index, ensuring the current player is
	 * excluded from selection.
	 * 
	 * @throws CannotDiscardException If the selected player has no cards to discard
	 *                                or if any other discard error occurs.
	 */
